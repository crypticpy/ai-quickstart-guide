...
- **Public code matching.** Enable "block suggestions matching public code" — reduces (does not eliminate) the risk of a license-incompatible suggestion.
...
- **Privacy mode.** Enabled by default for all users — disables retention of agency code on Cursor servers.
...
- **Attribution in commits when material.** When the bulk of a PR is AI-generated (an agent run, a refactor done by Claude Code), the commit message says so: a `Co-Authored-By:` trailer or a one-line note. This is professional courtesy and helps with future debugging.
...
- **Watch agentic costs.** A long Claude Code session that runs many tool calls is materially more expensive than inline completion. Budget accordingly.
...
- **Banning the tools.** Drives usage underground; loses audit and policy. Configure and train instead.
