...
def _key(system: str, user: str, model: str, kwargs: dict | None = None) -> str:
    """Cassette key derived from every input that can change the response.

    System, user, and model are the obvious ones. `kwargs` covers everything
    else the caller passes through (temperature, max_tokens, tools, ...). Two
    tests with the same prompt but different temperatures must not share a
    cassette entry.
    """
    h = hashlib.sha256()
    ...
    h.update(b"\x00")
    h.update(json.dumps(kwargs or {}, sort_keys=True, default=str).encode())
