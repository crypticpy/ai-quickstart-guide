"""Tests for the lab 4.4 permit-status agent.

The agent loop tests use a stub client so the lab can run without API access.
The stub returns a scripted sequence of model responses, one per iteration.
"""
...
# ---------------------------------------------------------------------------
# Agent loop tests with a scripted stub client
# ---------------------------------------------------------------------------
...
def test_agent_stops_at_max_iterations():
    # Always return a tool_use; the loop should bail out at max_iterations.
    looping = [
        StubResponse(
            content=[
                StubBlock(
